            ...
            <div class="input-info">
                ...
                <div>
                    ...
                    <select v-model="user.role">
                        <option v-for="(item, index) in listRole" :value="item['代码']" :key="index">{{item['角色名称']}}</option>
                    </select>
                </div>
                <div>
                    <span>设置密码:&#12288;</span>
...
export default {
  ...
  methods: {
    ...
    setUser (info) {
      ...
      console.log('用户', info)
      this.$api.application.findUserTree({
        jgmcm: info['机构名称码']
      }).then(data => {
        this.powerRange = data
      })
    },
    conditionSearch1 () {
    ...
    getData () {
      this.$api.application.unitsTree({
        ...
        let data1 = []
        this.queryList(this.data, data1)
      })
    },
    ...
    sureAdd () {
      console.log(this.receiver[this.receiver.length - 1])
      let date = new Date()
      ...
      this.$api.systemSet.addUser({
        ...
        jgmcm: this.receiver[this.receiver.length - 1],
