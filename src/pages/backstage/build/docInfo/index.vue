  ...
  <div class="stage">
    ...
    <div>
      <div class="left">
        ...
        <div class="type">
          <div>
            <input type="radio" name="type" checked @change="changeType(1)" class="point" />
            <span>十分法</span>
          </div>
          <div>
            <input type="radio" name="type" @change="changeType(0)" class="point" />
            <span>二分法</span>
          </div>
        </div>
        ...
        <button v-if="tenTree && build" class="birth" @click="birthTree">生成目录</button>
        <PositionHorn />
      </div>
      <div class="right" v-show="isShow == true">
        <div class="top">
          <div class="top_btn">
            ...
            <button class="add" @click="finishBuild" v-show="wanchengjiandang">完成建档</button>
            <!-- <button
              @click="scanMaterial"
          ...
          <div class="top_btn1">
            ...
            <button class="button2" @click="previewDir">预览目录</button>
            <button class="point button3" @click="shAdd(3)">申请添加专有属性</button>
          </div>
          <PositionHorn />
        </div>
        <div class="bottom-left">
          ...
          <div class="bottomButtons">
            ...
            <div class="iconBox" v-show="!isScan">
            ...
            <el-button type="success" class="dragBtn" v-show="!editImgShow && !isScan" @click="completeEditImg">完成</el-button>
            <el-button type="primary" class="dragBtn" v-show="editImgShow && !isScan" @click="editImg">编辑</el-button>
            <el-button type="success" class="dragBtn" v-show="dragEnabled && !isScan" @click="completeDragImg">完成</el-button>
            <el-button type="primary" class="dragBtn" v-show="!dragEnabled && !isScan" @click="dragImg">排序</el-button>
            <el-button id="btnScan" type="primary" class="dragBtn" v-show="isScan" @click="scanUploadmulu()">上传至目录</el-button>
            <el-button id="btnScan" type="primary" class="dragBtn" v-show="isScan" @click="AcquireImage()">扫描</el-button>
            <el-button
              type="primary"
              v-show="!isScan" 
            ...
            >上传至目录</el-button>
            <el-button type="primary" v-show="!isScan" class="dragBtn disableDragBtn" disabled="disabled">图像识别</el-button>
            <el-button type="primary" v-show="!isScan" class="dragBtn disableDragBtn" @click="imgOpt" disabled="disabled">图像优化</el-button>
          </div>
          <div class="left_img" v-show="isScan">
            <div id="DWTcontainer" class="container">
              <div id="dwtcontrolContainer"></div>
            </div>
          </div>
          <div class="left_img" v-show="!isScan" ref="left_img">
            <div class="imgs">
              <!-- 拖拽 -->
              <draggable
                class="list-group"
              ...
              <div class="firstMode" v-for="(item, index) in overviewList" :key="index">
                ...
                <!-- <ul>
                  <li class="modeLi" v-for="(v,i) in item.children" :key="i">
                    ...
                    <ul>
                      <li class="modeLi" v-for="(e,k) in item.children" :key="k">
...
export default {
  ...
  created() {
    ...
    Dynamsoft.WebTwainEnv.ProductKey = "t0068MgAAACmHcGX7qTsBn2Pu7//7HQN0POgIF4bpP0ZzdJIILpB6z8PBcETEVa2LDJ7D1qVFd10hkKR1diYfqau3VqfQgdA=";
    Dynamsoft.WebTwainEnv.Containers = [
      { ContainerId: "dwtcontrolContainer", Width: "930px", Height: "650px"}
    ];
  ...
  methods: {
    ...
    AcquireImage() {
      ...
      // this.DWObject.AcquireImage(this.optionalAsyncSuccessFunc);
      let scanIndex = 0
      this.DWObject.SetViewMode(2,2)
      this.DWObject.RegisterEvent('OnPostTransferAsync', () => {
        this.DWObject.ConvertToBase64 ([scanIndex], EnumDWT_ImageType.IT_PDF, this.asyncSuccessFunc, this.asyncFailureFunc);
        scanIndex++
      });
    },
    asyncFailureFunc (errorCode, errorString) {
      alert("ErrorCode: "+errorCode +"\r"+"ErrorString:"+ errorString);
    },
    asyncSuccessFunc (result) {
      var length=result.getLength();
      this.scanFormData.append('file', this.base64toFile(`data:image/png;base64,${result.getData(0,length)}`))
    },
    scanUploadmulu () {
      if(this.uuid) {
        this.$api.build.uploadFileToFuWuQi(this.scanFormData).then(data => {
          this.scanFormData.delete("file");
    ...
    imgUpload(e) {
      ...
      console.log('this.imgClassNameArray', this.imgClassNameArray)
    ...
    uploadmulu() {
      if (this.uuid) {
        ...
        if (this.imgUploadParams.length === 0) {
          ...
          formData.append("sfh", this.$route.query.sfh);
    ...
    getTree() {
      ...
      this.$api.build
        ...
        .then(data => {
          ...
          console.log('this.tenTree', this.tenTree)
    ...
    handleNodeClick(data) {
      ...
      this.DWObject.RemoveAllImages()
      // 更新扫描的formdata
      this.scanFormData = new FormData();
      this.scanFormData.append("sfh", this.$route.query.sfh);
      this.scanFormData.append("uuid", this.uuid);
      this.scanFormData.append("type", "uploadYuan");
      this.scanFormData.append("benrensfh", this.userInfo["身份号"]);
      this.treeSelectNode = data;
      if (data['大类代码']) {
        this.dialogDiv = false;
        this.isShowInput = true;
...
        this.$api.build
          .removeTree({
            sfh: this.treeSelectNode.身份号,
            cldm: this.treeSelectNode.材料代码,
            dah: this.$route.query.danganID,
            danwei: this.userInfo["部别"],
            czr: this.userInfo["身份号"],
            IP: "",
