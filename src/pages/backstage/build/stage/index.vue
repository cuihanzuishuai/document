...
<!--        <el-radio v-model="radio" label="1">显示包含下级单位</el-radio>-->
      ...
      <div class="right">
        <el-dialog
          ...
          :visible.sync="isJump"
          width="30%">
          <span>当前档案已建成，请到日常维护模块维护档案</span>
        ...
        <div class="top">
          ...
          <div class="search_btn"  v-if="isShowsql">
            <button class="search point" @click="conditionSearch" @keyup.enter="conditionSearch()">查询</button>
            <button class="clear_all point" @click="clearAll">清空</button>
          </div>
          <div class="search_btn" v-else>
            <button class="search point" @click="conditionSearch1">查询</button>
            <button class="clear_all point" @click="clearAll">清空</button>
          </div>
          <div>
        ...
        <div class="bottom">
          <div class="title_tab">
            ...
            <ul class="tab">
              <li class="point" @click="changeType('card')">
                <img src="../../../../assets/images/kapian-1.png" alt />
                <span>卡片</span>
              </li>
              <li class="point" @click="changeType('table')">
...
export default {
  ...
  methods: {
    sureJump () {
      this.$router.push('/home/backstage/docInfo?sfh=' + this.currentDoc['身份号'] + '&danganID=' + this.currentDoc['档案号'] + '&type=' + this.currentDoc['来源类别'] + '&name=' + this.currentDoc['姓名'] + '&danwei=' + this.currentDoc['部别'] + '&jgmcm=' + this.currentDoc['机构名称码']  + "&zhiwu=" + this.currentDoc['职务名称'] + '&daily=' + 1)
    },
    changeIfShowAll () {
      if (this.isAll === '2') {
    ...
    lookDoc(item) {
      ...
      if (item["纸质档案状态"] === "无数据") {
        ...
        /*判断 如果电子档案无数据 -- 则跳转建立档案模块的电子档案页面
                                -- 否则跳转日常维护模块的电子档案页面
        */
        if (item['电子档案状态'] !== '无数据') {
          this.isJump = true
          // this.$router.push(
          //   "/home/backstage/dailyCare/docInfo?sfh=" +
          //     item["身份号"] +
          //     "&danganID=" +
          //     item["档案号"] +
          //     "&type=" +
