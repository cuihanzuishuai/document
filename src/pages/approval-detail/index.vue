          ...
          <div class="status">
            ...
            <div class="status_1">
              <i
                ...
                v-if="$route.query.state === '已同意' || $route.query.state === '06'"
              ></i>
            ...
            <div class="step">
              <el-steps :active="logData.length" finish-status="success">
                <el-step
                  ...
                  :title="item['名称']"
                  :description="item['审批人姓名'] ? item['审批人姓名']+'('+item['审批单位']+')': ''"
                  :key="index"
                ></el-step>
              </el-steps>
            </div>
          ...
          <div class="approval_info">
            ...
            <div class="info">
              ...
              <ul class="right">
                ...
                <li>
                  <el-select v-model="days" :disabled="list['名称'] === '已审批'" style="width:100px;height:30px;">
                    <el-option v-for="(item, index) in dayArr" :value="item" :key="index">{{item === '' ? '请选择' : item}}</el-option>
                ...
                <li>
                  <el-select v-model="times" :disabled="list['名称'] === '已审批'" style="width:100px;height:30px;">
                    <el-option v-for="(item, index) in timeArr" :value="item" :key="index">{{item === '' ? '请选择' : item}}</el-option>
                  </el-select> 次
                </li>
              </ul>
            </div>
            <div class="opt_btn">
              ...
              <button class="submit" @click="submit" :disabled="check" v-if="this.ywbmld !=='业务部门领导'">提交上级</button>
...
export default {
  ...
  created() {
      this.ywbmld = JSON.parse(document.cookie.split("=")[1]).用户名;
    if (
      this.$route.query.type === "查阅电子档案" ||
      this.$route.query.type === "01"
    ) {
      this.type = "01";
    } else if (
      this.$route.query.type === "查阅纸质档案" ||
      this.$route.query.type === "02"
    ) {
      this.type = "02";
    } else if (
      this.$route.query.type === "借阅纸质档案" ||
      this.$route.query.type === "03"
    ) {
      this.type = "03";
    } else if (
      this.$route.query.type === "查看档案日志" ||
      this.$route.query.type === "04"
    ) {
      this.type = "04";
    }
    this.getProject();
  ...
  methods: {
    editImg() {
      this.$router.push(
        "/home/edit_doc_img?content=" +
          this.list["申请查档内容"] +
          "&docNames=" +
          JSON.stringify(this.list["查档对象"]) +
          "&shenpi=" +
          this.$route.query.shenqingID
      );
    },
    banli() {
