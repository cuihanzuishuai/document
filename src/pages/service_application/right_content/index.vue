  ...
  <div class="right_2">
    ...
    <div class="search_result common_style">
      <div class="title">
        <span>查询结果</span>
        <ul class="tab">
          <li @click="changeType('card')">
            <img src="../../../assets/images/kapian-1.png" alt class="point" />
            <span>卡片</span>
          </li>
          <li @click="changeType('table')">
            <img src="../../../assets/images/liebiao-1.png" alt class="point" />
            <span>列表</span>
          </li>
        </ul>
      </div>
        ...
        <div class="list">
          <div v-for="(item, index) in lists" :key="index" class="card">
            <div class="dentity">
              ...
              <span>{{item['职务名称']}}</span>
              <img
                src="../../../assets/images/junzhang.png"
                alt
                v-watermark="{text:'水印名称',textColor:'rgba(180, 180, 180, 0.3)'}"
              />
            </div>
              ...
              <div class="person_info">
                ...
                <ul class="list_info">
                  ...
                  <li>
                   ...
                   <li style="width:100px;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;">
        ...
        <el-table :data="lists" stripe style="width: 100%">
          ...
          <el-table-column prop="option" label="操作" width="210">
            <div slot-scope="scope">
              <span
                class="add_search point"
                v-if="types !== '04'"
                @click="addSearch(scope.row)"
              >加入查档人</span>
    ...
    <div class="select_file common_style">
      ...
      <div class="con">
        <el-table :data="checked" style="width: 100%">
          <el-table-column prop="姓名" label="姓名"></el-table-column>
          <el-table-column :prop="'部别'" label="单位"></el-table-column>
          <el-table-column prop="身份号" label="身份号"></el-table-column>
          <el-table-column :prop="'政治'" label="政治面貌"></el-table-column>
          <el-table-column prop="现管单位名称" label="现管部门"></el-table-column>
          <!-- <el-table-column
                        prop="职务名称"
                        label="职务"
                        width="80"
          ...
          <el-table-column prop="option" width="180" label="操作">
            <div slot-scope="scope">
      ...
      <div class="con">
        <div class="whthin_reason">
          <el-checkbox-group v-model="reason" @change="handleCheckedReasonChange">
            <el-checkbox
              v-for="(item, index) in condition['查档事由']"
              :label="item['名称']"
              name="type"
              :key="index"
            ></el-checkbox>
        ...
        <div class="whthin_form">
          ...
          <div>
            ...
            <el-checkbox-group v-model="form" @change="handleCheckedFormChange">
              <el-checkbox
                v-for="(item, index) in condition['申请服务类型']"
                :label="item['名称']"
                name="type"
                :key="index"
              ></el-checkbox>
        ...
        <div class="whthin_content" v-if="types !== '04'">
          ...
          <div>
            <el-checkbox-group v-model="con" @change="handleCheckedConChange">
              <el-checkbox
                v-for="(item, index) in condition['查档内容']"
                :label="item['名称']"
                name="type"
                :key="index"
              ></el-checkbox>
    ...
    <div class="submit_btn_2">
      ...
      <button class="submit point" @click="submit_chajieyue">提交申请</button>
      ...
      <div class="con con_4">
        ...
        <div class="application_info">
          ...
          <p>
            ...
            <span v-else>{{userInfo['姓名']}}</span>
          </p>
          <p>
            查档对象:
            <span v-for="(item, index) in bcdrxm" :key="index">{{item+ " "}}</span>
          </p>
          <p>申请编号: {{sqbh}}</p>
          <p>提交时间: {{appdata}}</p>
        </div>
      ...
      <div class="con con_3">
        ...
        <div class="project_name">
          ...
          <el-select
            ...
            v-model="fanganmc"
            style="margin-left:8px;width:300px;height:30px"
          >
        ...
        <div class="project_desc">
          <div class="application_info">
            <p>
              方案名称:
              <span>{{currentProject['方案名称']}}</span>
            </p>
            <p>
              查档对象:
              <span v-for="(i, key) in currentProject['方案查档对象']" :key="key">{{i+' '}}</span>
            </p>
      ...
      <div class="con con_3">
        ...
        <div class="select_1">
          ...
          <div class="project_desc">
            ...
            <el-select
              v-model="checked[checkedIndex].czlx"
              placeholder="请选择"
              multiple
              @change="changecztype(checkedIndex)"
            >
              <el-option v-for="item in czType" :key="item['名称']" :value="item['名称']"></el-option>
            </el-select>
    ...
    <div v-if="selfAddModel" class="save_model add">
      <div class="con con_3">
        ...
        <div>
          ...
          <p>
            <input type="text" v-model="addInfo.sfh" />
          </p>
        </div>
        <div>
          <p>
          ...
          <p>
            <input type="text" v-model="addInfo.danwei" />
          </p>
        </div>
        <div>
          <p>
          ...
          <p>
            <input type="text" v-model="addInfo.xgdanwei" />
          </p>
        </div>
        <div>
          <p>
          ...
          <p>
            <input type="text" v-model="addInfo.zm" />
          </p>
        </div>
        <div class="addbtn">
          <button class="cancel point" @click="cancel">取消</button>
    ...
    <el-dialog :visible.sync="dialogVisible" width="50%" center :modal-append-to-body="false">
      <div id="iframe">
        <h1
          style="text-align: center;font-size: 24px;margin-bottom: 20px;margin-top: 30px"
        >查（借）阅干部档案审批表</h1>
        <table style="margin: 0 auto;width: 80%;" border="1">
          <tr class="trcaption" style="width: 100%;height: 50px">
            <td class="linetd" style="width: 20%;text-align: center">项 目</td>
            <td class="linetd" style="width: 20%;text-align: center">姓 名</td>
            <td style="width: 30%;text-align: center;">部 职 别</td>
          ...
          <tr class="trcaption" v-for="(item, index) in tableMsg">
            <td
              class="linetd"
              v-if="index === 0"
              :rowspan="tableMsg.length"
              style="text-align: center"
            >
              查 借 阅
              <br />人 员
            </td>
            <td class="linetd" style="text-align: center;height: 60px;">
              {{item.姓名}}
              <!--               <ul>-->
            ...
            <td style="width: 40%;text-align: center;height: 60px;">{{item.部别}}{{item.职务名称}}</td>
            <td style="width: 40%;text-align: center;height: 60px;">{{item.政治}}</td>
          </tr>
          <tr class="trcaption" v-for="(item, index) in checked1">
            <td
              ...
              style="text-align: center"
              v-if="index === 0"
              :rowspan="checked1.length"
            >
              被 查 借 阅
              <br />人 员
            </td>
            <td class="linetd" style="text-align: center;height: 60px;">{{item.姓名}}</td>
            <td
              style="width: 40%;text-align: center;height: 60px;"
              colspan="2"
            >{{item.部别}}{{item.职务名称}}</td>
          </tr>
          ...
          <tr class="trcaption" style="height: 230px">
            ...
            <td class="linetd" colspan="3">
              <div style="margin-top: 80px;margin-left: 42px;float: left;">
                <span>负责人签字</span>
              </div>
              <div style="float: right;margin-right: 113px;margin-top: 20px;">
                <span style>（公 章）</span>
              </div>
              <div style="float: right;margin-right: -98px;margin-top: 70px;">
  ...
  methods: {
    ...
    changeProject() {
      this.fangan.forEach(item => {
        ...
        if (
          item["方案名称"].split(" ").join("") ===
          this.fanganmc.split(" ").join("")
        ) {
          // console.log('进来')
      ...
      this.currentProject["方案查档对象"] = JSON.parse(
        this.currentProject["方案查档对象"]
      );
      this.currentProject["方案查询条件"] = JSON.parse(
        this.currentProject["方案查询条件"]
      );
    },
    back() {
      this.$router.back();
    ...
    submit_chajieyue() {
      ...
      // }
      // console.log('this.$route.name !',this.$route.name !== 'FileLog')
      //       console.log('this.cdr.length >=2',this.cdr.length >=2)
      //        console.log('this.bcdr.length > 0',this.bcdr.length > 0)
      //         console.log('this.reason.length > 0',this.reason.length > 0)
      //          console.log('this.form.length > 0',this.form.length > 0)
      //           console.log('this.con.length > 0',this.con.length > 0)
      //           console.log('this.typeContent',this.typeContent)
      if (this.$route.name !== "FileLog") {
        ...
        if (
          ...
          if (
            this.start.length === this.checked.length &&
            this.end.length === this.checked.length
          ) {
            this.$api.application
              .submitApplication({
                danwei: this.userInfo["部别"],
